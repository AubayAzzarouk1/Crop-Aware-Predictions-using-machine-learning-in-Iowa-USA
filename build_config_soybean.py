...
def build_soybean_train(target_fips=None):
    ...
    # Filter rows so that only Iowa counties are selected.
    # Adjust "State" to the exact column name in your CSV.
    df = df[df["State"] == "IA"]

    if target_fips:
        df = df[df["FIPS Code"].isin(target_fips)]

    counties = df.to_json(orient='records', lines=False)
    counties = json.loads(counties)

    path = "/content/drive/MyDrive/MMST-ViT/data/soybean_train.json"
...
def build_soybean_val(target_fips=None):
    csv_path = "/content/drive/MyDrive/MMST-ViT/input/county_info_2022.csv"
    df = pd.read_csv(csv_path)

    # Filter only Iowa counties:
    df = df[df["State"] == "IA"]
    ...
    path = "/content/drive/MyDrive/MMST-ViT/data/soybean_val.json"
...
def get_json_obj(year, county_info):
    fips, county, state = str(county_info["FIPS Code"]), county_info["County"], county_info["State"]
