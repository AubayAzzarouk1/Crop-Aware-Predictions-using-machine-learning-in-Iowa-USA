...
class MMST_ViT(nn.Module):
    ...
    def forward(self, x, ys=None, yl=None):
        ...
        # Skip flattening if yl already has shape [b, y*m*d]
        # So we assume yl is already [b, y*m*d] (e.g., [1, 12636])
        # Validate and reshape yl
        print(f"🔍 Original yl shape: {yl.shape}")
        if yl.ndim == 4:
            yl = rearrange(yl, 'b y m d -> b (y m d)')
            print(f"📦 Flattened yl to: {yl.shape} for projection")
            
        yl = self.proj_context(yl)  # [b, t*d]
        print(f"🎯 After proj_context: {yl.shape}")
        yl = rearrange(yl, 'b (t d) -> b t d', t=t)
        print(f"🧩 Reshaped yl to match temporal transformer input: {yl.shape} with t = {t}")

        assert yl.shape[1] == t, f"❌ Temporal mismatch: expected t={t}, got {yl.shape[1]}"
